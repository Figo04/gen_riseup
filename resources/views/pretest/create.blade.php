    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 text-sm rounded-lg p-4">
                Jawaban pre-test bersifat final dan tidak dapat diubah setelah dikirim. Pastikan semua pertanyaan terjawab sebelum menekan "Kirim Pre-Test".
            </div>

            <form method="POST" action="{{ route('pretest.store') }}" class="space-y-6">
                @csrf
                ...
                <div class="flex justify-end">
                    <x-primary-button>{{ __('Kirim Pre-Test') }}</x-primary-button>
                </div>
            </form>
        </div>
    </div>
