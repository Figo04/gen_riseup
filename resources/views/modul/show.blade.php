    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            @foreach ($modul->subBagian->sortBy('urutan') as $sub)
                <a href="{{ route('modul.sub-bagian.show', [$modul, $sub]) }}" class="flex items-center justify-between bg-white shadow-sm sm:rounded-lg p-6 hover:bg-gray-50">
                    <span class="text-gray-800">{{ $sub->judul }}</span>
                    @if ($selesaiIds->contains($sub->id))
                        <span class="text-sm text-green-600">✓ Selesai</span>
                    @else
                        <span class="text-sm text-gray-400">Belum selesai</span>
                    @endif
                </a>
            @endforeach
        </div>
